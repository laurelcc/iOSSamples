...
jQuery(document).ready( function($) {
	...
	//Remember and register form AJAX
 	...
 	$(document).on('lwa_login', function(event, data, form){
 		...
 		// Force redirect modal login
 		var force = 'no';
 		if( typeof( form.parent().data('redirect') ) != "undefined" && form.parent().data('redirect') !== null ) {
			force = form.parent().data('redirect');
		}

		if(data.result === true){
			//Login Successful - Extra stuff to do
			if( data.widget != null && force === 'no' ){
				$.get( encodeURI(data.widget), function(widget_result) {
	...
	//Registration overlay
 	$('.lwa-modal').each( function(i,e){
 		...
 		/**
 		 * MH Custom
 		 * remove lwa modal class for custom style
 		 */
 		modal.removeClass('lwa-modal');
